...
export const createAddress = (formdata, id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API}/api/v1/me/address`,
      formdata,
      config
    );
    dispatch({
...
export const allAddress = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/me/addresses`,
      { withCredentials: true }
    );
    dispatch({
...
export const singleAddress = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/me/address/details/${id}`,
      { withCredentials: true }
    );
    dispatch({
...
export const deleteAddress = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${process.env.REACT_APP_API}/api/v1/me/address/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const AdminallAddress = (id) => async (dispatch) => {
  try {
    dispatch({ type: ALL_ADDRESS_REQUEST });
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/me/addresses/${id}`,
      { withCredentials: true }
    );
    dispatch({
      type: ALL_ADDRESS_SUCCESS,
