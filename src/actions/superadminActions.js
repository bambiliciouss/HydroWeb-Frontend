...
export const allUsersSA = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/superadmin/allusers`,
...
export const allPendingSA = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/superadmin/all/pendingadmin`,
      { withCredentials: true }
    );
    console.log(data);
    dispatch({
...
export const allAdminSA = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/superadmin/all/admin`,
      { withCredentials: true }
    );
    console.log(data);
    dispatch({
