...
export const createProduct = (productData, id) => async (dispatch) => {
  ...
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API}/api/v1/admin/create/product/${id}`,
      productData,
      config
    );
    dispatch({
  ...
  (productid, stockid) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API}/api/v1/admin/product/${productid}/stocklogs/delete/${stockid}`,
        {
          withCredentials: true,
        }
      );
...
export const getProductDetails = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API}/api/v1/admin/all/product/${id}`,
      config
    );
...
export const updateProductDetails = (id, productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_API}/api/v1/admin/update/product/price/${id}`,
      productData,
      config
    );
    dispatch({
...
export const deleteProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `${process.env.REACT_APP_API}/api/v1/admin/product/delete/${id}`,
      {
        withCredentials: true,
      }
    );
